...
@objc open class AXOverlayTopView: UIView, AXStackableViewContainerDelegate {
    
    #if os(iOS)
    /// The toolbar used to set the `titleView`, `leftBarButtonItems`, `rightBarButtonItems`
    @objc public let toolbar = UIToolbar(frame: CGRect(origin: .zero, size: CGSize(width: 320, height: 24)))
...
//
       ...
       // self.toolbar.backgroundColor = .clear
        ...
        self.bottomStackContainer = AXStackableViewContainer(views: [], anchoredAt: .top)
       // self.bottomStackContainer.backgroundColor = AXConstants.overlayForegroundColor
        self.bottomStackContainer.backgroundColor = .clear
...
//        self.topStackContainer.frame = CGRect(origin: .zero, size: self.topStackContainer.sizeThatFits(self.frame.size))
        ...
        let topInset:CGFloat = (self.contentInset.top > 40) ? 20 : 0;
        
        self.bottomStackContainer.contentInset = UIEdgeInsets(top: topInset,
                                                              left: self.contentInset.left,
                                                              bottom: 0,
                                                              right: self.contentInset.right)
        let bottomStackSize = self.bottomStackContainer.sizeThatFits(self.frame.size)
        self.bottomStackContainer.frame = CGRect(origin: CGPoint(x: 0, y: 0),
...
//    }
    ...
    func updateCaptionView(photo: AXPhotoProtocol) {
        ...
        let animations = { [weak self] in
            ...
            self.bottomStackContainer.frame = CGRect(origin: CGPoint(x: 0, y: 0), size: size)
