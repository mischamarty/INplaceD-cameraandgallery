...
public class ABVideoRangeSlider: UIView, UIGestureRecognizerDelegate {
    ...
    private func setup(){
        ...
        leftView = UIView(frame: CGRect(x: 0, y: 0, width: 0, height: 35))
        leftView.backgroundColor = .black
        leftView.alpha = 0.75
        self.addSubview(leftView)
        
        rightView = UIView(frame: CGRect(x: 0, y: 0, width: 0, height: 35))
...
//                                                              width: 10,
//                                                              height: self.frame.size.height + bottomBorderHeight + topBorderHeight))
        progressIndicator = ABProgressIndicator(frame: CGRect(x: 30,
                                                              y: -2.5,
                                                              width: 10,
                                                              height: 44.5))
        ...
        startTimeView = ABTimeView(size: CGSize(width: 130, height: 30), position: 1)
    ...
    public func hideOnlyProgressIndicator(){
           DispatchQueue.main.async {
                      //self.progressIndicator.isHidden = true
       ...
       public func showOnlyProgressIndicator(){
           DispatchQueue.main.async {
                      //self.progressIndicator.isHidden = false
    ...
    public func updateThumbnails(){
        if !isUpdatingThumbnails{
            ...
            let backgroundQueue = DispatchQueue(label: "com.app.queue2", qos: .background, target: nil)
    ...
    private func processHandleDrag(
        ...
        if recognizer.state == .began {
                  hideOnlyProgressIndicator()
              }
              
              if recognizer.state == .ended {
                  showOnlyProgressIndicator()
              }
    ...
    public func secondsFromValue(value: CGFloat) -> Float64{
...
//        self.layer.cornerRadius = 5
        ...
        startTimeView.timeLabel.text = self.secondsToFormattedString(totalSeconds: secondsFromValue(value: self.startPercentage)) + " / " +
            self.secondsToFormattedString(totalSeconds: secondsFromValue(value: self.endPercentage))
        ...
        startIndicator.center = CGPoint(x: startPosition, y: startIndicator.center.y)
        endIndicator.center = CGPoint(x: endPosition, y: endIndicator.center.y)
        progressIndicator.center = CGPoint(x: progressPosition + 4, y: progressIndicator.center.y)
        ...
        leftView.frame = CGRect(x: 0,y: 0, width: startIndicator.frame.origin.x, height: self.frame.height)
        rightView.frame = CGRect(x: endIndicator.frame.origin.x,y: 0, width: self.frame.width - endIndicator.frame.origin.x, height: self.frame.height)

        
//        rightView.frame = CGRect(x: endIndicator.frame.origin.x,
//              y: 0,
//              width: self.view. startIndicator.frame.origin.x,
//              height: self.frame.height)
        ...
        startTimeView.center = CGPoint(x: startIndicator.frame.origin.x + startTimeView.frame.width/2, y: 60)
        endTimeView.center = CGPoint(x: startIndicator.center.x + startTimeView.frame.size.width, y: 35)
    }
    ...
    public func secondsToFormattedString(totalSeconds: Float64) -> String{
